...
static volatile int* MASKR[CPU_NUMS][MASKR_NUMS] = {
    &MASKR(0), &MASKR(1), &MASKR(2), &MASKR(3), &MASKR4};

static volatile int* QSTR[CPU_NUMS][MASKR_NUMS] = {&QSTR(0), &QSTR(1), &QSTR(2),
                                                   &QSTR(3), &QSTR4};

#define SET_TR_CRAM() CSR |= 2
#define UNSET_TR_CRAM() CSR &= ~(2)
